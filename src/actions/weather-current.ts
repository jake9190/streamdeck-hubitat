...
export class WeatherCurrent extends SingletonAction<WeatherCurrentSettings> {
	...
	private async render(
		...
		const svg  = `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
			...
			<text x="72" y="54" text-anchor="middle" font-size="56" fill="${emojiColor}">${emoji}${alert}</text>
			<text x="72" y="93" text-anchor="middle" font-family="Arial,sans-serif" font-size="28" fill="white" font-weight="bold">${temp}° ${humidity}%</text>
